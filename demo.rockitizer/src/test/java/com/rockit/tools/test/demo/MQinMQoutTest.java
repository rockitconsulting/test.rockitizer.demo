...
public class MQinMQoutTest extends AbstractTestWrapper {
	public static Logger logger = Logger.getLogger(MQinMQoutTest.class
			.getName());
	...
	public void testAddMsgToMQFlow() throws Exception {
		...
		// do assertion with recorded values if mode is reply
				testBuilder.addAssertion(new XMLFileAssertion("a002MQGetError")
		...
		// do assertion with recorded values if mode is reply
		testBuilder.addAssertion(new XMLFileAssertion("a002MQGETMessage")
				.withNodeMatcher(ElementSelectors.byNameAndText)
				//.ignoreAttrs(ImmutableList.of("updateTimestamp", "insertTimestamp","processingTime"))
				//.ignore(ImmutableList.of("updateTimestamp", "insertTimestamp","beginTimestamp", "endTimestamp", "addressDate","businessConnectionDate"))
				.checkForSimilar());
